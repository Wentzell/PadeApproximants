...
namespace triqs {
  namespace utility {
    ...
    class pade_approximant {
      ...
      triqs::arrays::vector<dcomplex> z_in; // Input complex frequency points
      triqs::arrays::vector<dcomplex> a;    // Pade coefficients

      public:
      static const int GMP_default_prec = 256; // Precision of GMP floats to use during a Pade coefficients calculation.

      pade_approximant(const triqs::arrays::vector<dcomplex> &z_in_, const triqs::arrays::vector<dcomplex> &u_in) : z_in(z_in_), a(z_in.size()) {

        using details::gmp_complex;
        ...
        triqs::arrays::array<gmp_complex, 2> g(N, N);
      ...
      dcomplex operator()(double e) const {
	return operator()(dcomplex(e, 0.)); 
      }
    };
